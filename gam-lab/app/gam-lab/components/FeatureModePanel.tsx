...
export default function FeatureModePanel({ trainData, shapes, featureModes, onSetFeatureMode }: Props) {
  ...
  const importanceByKey = useMemo(() => {
    ...
    shapes.forEach((s) => {
      if (s.editableZ) {
        // RMS of the 2D grid as a proxy for interaction importance
        const vals = s.editableZ.flat();
        raw[s.key] = vals.length
          ? Math.sqrt(vals.reduce((sum, v) => sum + v * v, 0) / vals.length)
          : 0;
      } else {
      ...
      <div className={styles.featureModeRows}>
        {sortedRegularKeys.map((key) => {
          ...
          return (
            <div
              key={key}
              className={`${styles.featureModeRow} ${isDeactivated ? styles.featureModeRowDeactivated : ""}`}
            >
              ...
              <div className={styles.featureModeControls}>
                <label className={styles.featureModeLockLabel} title="Lock: freeze shape, skip boosting">
                  ...
                  />
                  <span className={styles.featureModeLockTrack}>
                    <span className={styles.featureModeLockThumb} />
                  </span>
                ...
                <label className={styles.featureModeDeactivateLabel} title="Off: remove from model entirely">
                  <input
                    ...
                    onChange={(e) => {
                      onSetFeatureMode(key, e.target.checked ? "deactivate" : undefined);
                    }}
                  />
                  <span className={styles.featureModeDeactivateTrack}>
                    <span className={styles.featureModeDeactivateThumb} />
                  </span>
                  ...
                  {hist.type === "continuous" ? (
                    <>
                      <div className={styles.featureDistributionHistogram}>
                        <FeatureMiniHistogram bins={hist.bins} />
                      </div>
        ...
        {/* Interaction shapes — can be deactivated but not locked */}
        {interactionShapes.map((shape) => {
          ...
          return (
            <div key={shape.key} className={`${styles.featureModeRow} ${isDeactivated ? styles.featureModeRowDeactivated : ""}`}>
              ...
              <div className={styles.featureModeHist}>
                <div className={styles.featureDistributionHistogram}>
                  <InteractionHeatmap shape={shape} width={200} height={46} />
                </div>
              </div>
            </div>
          );
